...
class Data():
    ...
    def process(self):
        ...
        embs = {}
        with open(self.embs,encoding='utf-8') as f:
        ...
        print('Pairs容量:',pairs.size(0))
        print('种子个数',self.each_num)
        self.train_set = pairs[:int(self.rate*self.each_num)].to(self.device)
        self.eval_set = pairs[int(self.rate*self.each_num):int((self.rate+0.05)*self.each_num)].to(self.device)
        self.test_set = pairs[int(self.rate*self.each_num):].to(self.device)
    ...
    def process_graph(self, ids, names, triples, embs):
        ...
        with open(triples, 'r',encoding='utf-8') as f:
            edges = [list(map(int, line.strip().split('\t')))[0:3] for line in f.readlines()]
